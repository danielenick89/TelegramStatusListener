...
angular.module('myApp.controllers', ['myApp.i18n'])
  ...
  .controller('AppImHistoryController', function ($scope, $location, $timeout, $modal, $rootScope, MtpApiManager, AppUsersManager, AppChatsManager, AppMessagesManager, AppPeersManager, ApiUpdatesManager, PeersSelectService, IdleManager, StatusManager, NotificationsManager, ErrorService,StatusHistory) {
    ...
    $scope.yAxisTickFormatFunction = function() {
        return function(d, i) {
            ...
            else if(d == 0) return "offline";
            else return "";
        };
    }


    StatusHistory.subscribe(function(history) {
        //console.log(history);
        $scope.statusHistoryLength = Object.keys(history).length;
        
        if($scope.statusHistoryLength > 0 && peerID && history[peerID]) {
            var toPlot = history[peerID];
            ...
            var data = [{
                key: peerID,
                values: toPlot.map(function(e) {return {x: (e.timestamp+'').length == 10 ? e.timestamp*1000 : e.timestamp, y: e.status ? 1 : 0, size: 2 }})
            }];
