...
.service('StatusHistory', function ($rootScope, $modal, $modalStack, $filter, $q, qSync, MtpApiFileManager, MtpApiManager, RichTextProcessor, ErrorService, Storage, _, AppUsersManager) {
    ...
    var pushNewOnlineStatus = function(user,status,timestamp) {
        if(!history[user]) history[user] = []
        history[user].push({timestamp: timestamp, status:status});
        if(history[user].length > HISTORY_LENGTH) history[user].shift();
        
        listener && listener(history);
    ...
    var init = function() {
        ...
        $rootScope.$on('apiUpdate', function (e, update) {
            console.log('update intercepted',update)
            if(update._ == 'updateUserStatus' && update.user_id != myID) {
              pushNewOnlineStatus(update.user_id, (update.status._ == 'userStatusOnline'),tsNow());
            }
        });
        
        $rootScope.$on('user_update', function (e, userID) {
            var user = AppUsersManager.getUser(userID);
            
            console.log('update intercepted: user_update',userID)
            if(userID != myID) {
                if(user.status && (user.status._ == 'userStatusOnline' || user.status._ == 'userStatusOffline')) {
                    pushNewOnlineStatus(userID,user.status._ == 'userStatusOnline',tsNow(true));
                }
            }
        });
    ...
    return {
        pushNewOnlineStatus:pushNewOnlineStatus,
        subscribe:subscribe
    };
